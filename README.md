...
![Screenshot_1](https://telegra.ph/file/337e454d0593860328494.png)

# 🟣 LimeCoin AutoBot /// HPV /// V1.07 🟣

| **Возможности**                                                    | **Поддерживается**  |
|----------------------------------------------------------------|:---------------:|
| **Многофункциональный** — *выполняет все те же действия, что вы делали бы руками:*<br>**—** *Получение ежедневной награды*;<br>**—** *Улучшение бустов ("Сила клика" и "Лимит энергии")*;<br>**—** *Получение кол-ва бустов и их активация*;<br>**—** *Беспрерывное тапанье под бусты, до истечения дневного лимита*;<br>**—** *Ожидание 24 часа*;<br>**—** *Повторение действий через 24 часа*. |✅|
...
| **Proxy** — *скрипт поддерживает добавление неограниченного количества прокси-серверов. Даже если количество прокси меньше, чем количество аккаунтов, они будут равномерно распределены между всеми аккаунтами после проверки на валидность. Хотя использование прокси не является обязательным, настоятельно рекомендуется их использование для предотвращения блокировки в LimeCoin (не в Telegram)* |✅|
| **Точечная настройка** — *скрипт позволяет настроить автоматическое обновление бустов до заданного уровня. По умолчанию бусты "Сила клика" и "Лимит энергии" будут обновлены до уровня 9, что считается оптимальным решением. Изменение этого значения возможно, но не рекомендуется!* |✅|
...
- **Linux**
  ...
  - ```
    ...
    ```
  - ```
    git clone https://github.com/A-KTO-Tbl/LimeCoin
    ```
  - ```
    pip3 install -r LimeCoin\Core\Tools\requirements.txt
    ```
- **Windows**
  ...
  - Запустится CMD в текущей директории. Далее просто вводим следующие команды:
    - ```
      ...
      ```
    - ```
      pip install -r LimeCoin\Core\Tools\requirements.txt
      ```
- **Android**
